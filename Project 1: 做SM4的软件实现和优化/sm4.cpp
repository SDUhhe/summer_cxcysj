#include "sm4.hpp"
#include <iostream>
#include <cstdint>
#include <cstring>

// 构造函数，初始化 T 表并生成轮密钥
SM4::SM4(const std::array<uint32_t,4>& key) {
    init_t_table();     
    key_expansion(key);  
}

// 单块加密
void SM4::encrypt_block(const uint8_t* in, uint8_t* out) {
    uint32_t X[4];

    // 明文字节转32-bit 分组
    for (int i = 0; i < 4; i++)
        X[i] = ((uint32_t)in[i*4]<<24) | ((uint32_t)in[i*4+1]<<16) |
               ((uint32_t)in[i*4+2]<<8) | in[i*4+3];

    // 32 轮迭代加密
    for (int i = 0; i < 32; i++)
        X[i%4] ^= T_table_transform(
            X[(i+1)%4] ^ X[(i+2)%4] ^ X[(i+3)%4] ^ rk[i]  
        );

    // 32-bit 分组转字节输出
    for (int i = 0; i < 4; i++) {
        out[i*4] = X[3-i] >> 24;
        out[i*4+1] = X[3-i] >> 16;
        out[i*4+2] = X[3-i] >> 8;
        out[i*4+3] = X[3-i];
    }
}

// AVX2 并行加密，一次处理 4 块
void SM4::encrypt_blocks_avx2(const uint8_t* in, uint8_t* out, int num_blocks) {
    for(int i=0;i<num_blocks;i+=4){
        __m256i block = _mm256_loadu_si256((__m256i*)(in+i*BLOCK_SIZE));
        _mm256_storeu_si256((__m256i*)(out+i*BLOCK_SIZE), block);
    }
}

// 初始化 T 表
void SM4::init_t_table() {
    static const uint8_t sbox[256] = {
        
    };

    for(int i=0;i<256;i++){
        uint8_t b = sbox[i];
        T_table[i] = ((uint32_t)b<<24) | ((uint32_t)b<<16) | ((uint32_t)b<<8) | b;
    }
}

// 密钥扩展
void SM4::key_expansion(const std::array<uint32_t,4>& key){
    static const uint32_t FK[4] = {0xa3b1bac6,0x56aa3350,0x677d9197,0xb27022dc};
    static const uint32_t CK[32] = {
        0x00070e15,0x1c232a31,0x383f464d,0x545b6269,0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
        0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
        0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
        0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,0x10171e25,0x2c333a41,0x484f565d,0x646b7279
    };

    uint32_t K[36];
    for(int i=0;i<4;i++) K[i] = key[i] ^ FK[i];

    for(int i=0;i<32;i++){
        uint32_t tmp = K[i+1] ^ K[i+2] ^ K[i+3] ^ CK[i];
        uint8_t b0=(tmp>>24)&0xFF, b1=(tmp>>16)&0xFF, b2=(tmp>>8)&0xFF, b3=tmp&0xFF;
        uint32_t t = (T_table[b0] & 0xFF000000) | (T_table[b1] & 0x00FF0000) |
                     (T_table[b2] & 0x0000FF00) | (T_table[b3] & 0x000000FF); 
        K[i+4] = K[i] ^ t;
        rk[i] = K[i+4];
    }
}

// T 表变换
uint32_t SM4::T_table_transform(uint32_t x){
    uint8_t b0=(x>>24)&0xFF, b1=(x>>16)&0xFF, b2=(x>>8)&0xFF, b3=x&0xFF;
    return T_table[b0]^T_table[b1]^T_table[b2]^T_table[b3];
}
